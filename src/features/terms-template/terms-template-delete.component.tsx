...
import { useClassDispatch } from "./terms-template-list.context"
...
export default function DeleteTemplate({ id, close, opened, onClose }: DeleteProps) {
    ...
    const deleteTemplate = async () => {
        ...
        const onSuccess = ({ data: { data } }: ResponseClassflow<TermTemplate>) => {
            dispatch({
                type: "delete",
                payload: id
            })
        ...
        let del = new ClassflowDeleteService<{}, TermTemplate, string>(`/term-templates/${id}`, {});
        del.onSend = onSend;
        del.onError = onError;
