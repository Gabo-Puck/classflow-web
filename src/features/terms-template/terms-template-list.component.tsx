...
import { TermTemplateListItem, useClassDispatch, useTemplates, useQuery } from "./terms-template-list.context"
...
import DeleteTemplate from "./terms-template-delete.component";
...
export default function TermTemplateTable() {
    ...
    const onSuccess = ({ data, status }: ResponseClassflow<TermTemplateListItem[]>) => {
        dispatch({
            type: "set",
            payload: data.data
        });
    ...
    const getTemplates = async () => {
        ...
        let get = new ClassflowGetService<null, TermTemplateListItem[], string>(url, {});
        get.onSend = onSend;
        get.onError = onError;
