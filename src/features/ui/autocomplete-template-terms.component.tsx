...
interface AutocompleteTermsProps {
    url: string
    onSelect(s: Pick<TermTemplate, "id" | "name">): void
    selectedList: Pick<TermTemplate, "id" | "name">[] | null
}

export function AutocompleteTerms({ url, onSelect, selectedList }: AutocompleteTermsProps) {
    ...
    function getTerms(searchQuery: string, signal: AbortSignal) {
        ...
        return new Promise<Pick<TermTemplate, "id" | "name">[]>(async (resolve, reject) => {
            ...
            let get = new ClassflowGetService<{ email: string }, Pick<TermTemplate, "id" | "name">[], string>(url, {
            }, {
                email: searchQuery
            });
            get.onSend = onSend;
...
export const AutocompleteTermsTemplate = ({ onSelect, selectedList }: Omit<AutocompleteTermsProps, "url">) => <AutocompleteTerms url="/term-templates" onSelect={onSelect} selectedList={selectedList} />;
